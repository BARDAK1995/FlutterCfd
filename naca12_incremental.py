...
def nfunction(x,Def,V,parameters,Less_Iteration):
  #Take parameters.
...
#rhapson method.
def NewtonNumeric(xi,errcriteria,itcriteria,Def,V,parameters):
  N = xi.shape[0]
  x = xi
  i = 0
  maxerr = 1000
  with open('Incremental.csv', 'w', newline='') as csvfile:
    # Create a CSV writer object
      writer = csv.writer(csvfile)

      # Write the header row
      writer.writerow(['Iteration Number', 'V', 'h_amplitude', 'h_phase','Frequency','Damping','Error1','Error2','Error3','Error4'])
  #Define residuals without perturbation for numerical derivative using
  #forward difference.
  R = nfunction(x,Def,V,parameters,0)
  #Perturbation size.
  h = 1e-8
  #Relaxation.
  lamda = 1
  ...
  while (itcriteria>i) and (maxerr>errcriteria):
    ...
    for j in range(N):
      xh = x.copy()
      xh[j] = x[j]+h
      Rif = nfunction(xh,Def,V,parameters,1)
    ...
    with open('Incremental.csv', 'a', newline='') as csvfile:
    # Create a CSV writer object
      writer = csv.writer(csvfile)
      ...
      # Write the data rows
      writer.writerow([i,V,x[0][0],x[1][0]/np.pi*180,x[2][0]/2/np.pi,x[3][0],abserr[0],abserr[1],abserr[2],abserr[3]])
  roots = x
  iterationnumber = i

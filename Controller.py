...
@author: gunes
"""
import naca12 as structural_solver
import numpy as np
from math import pi
...
xi = np.array([[b*(1+a)*np.sin(deflection)*np.cos(-54*pi/180)], [b*(1+a)*np.sin(deflection)*np.sin(-54*pi/180)], [np.sqrt(Ka/I)], [70]])
...
roots, maxerr, iterationnumber = structural_solver.NewtonNumeric(xi, errcriteria, itcriteria, deflection, parameters)
